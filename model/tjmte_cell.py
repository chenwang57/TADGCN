...
class TimeAwarePeriodicityJointAttention(tf.keras.Model):
    def __init__(self, dimension, seq_out, heads=8, head_dim=64, dropout=0., is_multi_view=True, time_interval=5):
        super().__init__()
    ...
    def time_step_dist_calc(self, time_x, time_y):
        ...
        steps_dist = tf.where(minutes_dist > 720, 1440 - minutes_dist, minutes_dist) // self.time_interval
...
class TJMTECell(tf.keras.Model):
    def __init__(self, dimension, seq_in, seq_out, heads, head_dim, dropout=0., is_multi_view=True, time_interval=5):
        super().__init__()
        self.seq_in = seq_in
